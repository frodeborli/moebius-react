...
/**
 * An event loop which uses Moebius Coroutines to provide the classic event loop
 * functionality in React.
 *
 * This event loop implementation will utilize any event-loop extenstion that
 * moebius supports.
 */
final class MoebiusEventLoop extends Kernel implements LoopInterface
{
    ...
    /**
     * Holding coroutines when react loop is stopped
     */
    ...
    public function addReadStream($stream, $listener)
    {
        ...
        if (!key_exists($id, $this->readStreams)) {
            // React's event loop ignores this if an event listener already exists
...
//                Co::suspend();
                ...
                for (;;) {
                    ...
                    Co::readable($stream);
                    if (empty($this->readStreams[$id]) || !is_resource($stream)) {
                        break;
                    }
                    try {
                        $this->checkRunnable();
    ...
    public function addWriteStream($stream, $listener)
    {
        ...
        if (!key_exists($id, $this->writeStreams)) {
            // React's event loop ignores this if an event listener already exists
...
//                Co::suspend();
    ...
    public function addTimer($interval, $callback)
    {
        ...
        Co::go(function() use ($timer) {
            $this->stoppableSleep($timer->getInterval());
...
*/
    ...
    public function run()
    {
        if (count($this->stopped) > 0) {
            foreach ($this->stopped as $id => $co) {
    ...
    private function stoppableSleep(float $timeout): void {
        $stopTime = microtime(true) + $timeout;
        for (;;) {
            $remainingTime = $stopTime - microtime(true);
            if ($remainingTime <= 0) {
                return;
            }
            Co::sleep(min(0.1, $remainingTime));
            $this->checkRunnable();
        }
    }
    ...
    private function checkRunnable(): void {
        if (!$this->runnable) {
            if (self::$current) {
                $this->stopped[self::$current->id] = self::$current;
